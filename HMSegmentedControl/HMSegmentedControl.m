...
- (void)changSeclectionIndeicator:(float)rate{
//    [CATransaction begin];
//    [CATransaction setAnimationDuration:0.01f];
...
//    self.selectedSegmentLayer.frame = [self frameForSelectionIndicator];
    ...
    if (self.selectionIndicatorMode == HMSelectionIndicatorResizesToStringWidth) {
        CGFloat widthTillEndOfSelectedIndex = (self.segmentWidth * rate) + self.segmentWidth;
        CGFloat widthTillBeforeSelectedIndex = (self.segmentWidth * rate);
        
        CGFloat x = ((widthTillEndOfSelectedIndex - widthTillBeforeSelectedIndex) / 2) + (widthTillBeforeSelectedIndex - stringWidth / 2);
         
        self.selectedSegmentLayer.frame = CGRectMake(x, 0.0, stringWidth, self.selectionIndicatorHeight);
    } else {
        self.selectedSegmentLayer.frame =  CGRectMake(self.segmentWidth * rate, 0.0, self.segmentWidth, self.selectionIndicatorHeight);
    }
}
