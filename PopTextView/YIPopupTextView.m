...
- (void)showInView:(UIView*)view
{
    ...
    if (!view || [view isKindOfClass:[UIWindow class]]) {
        DEBUGLog(@"Warning: show in window currently doesn't support orientation.");
    }
    ...
    UIBarButtonItem *done = [[UIBarButtonItem alloc] initWithTitle:@"提交" style:UIBarButtonItemStyleDone target:self.target action:self.action];
...
- (void)didReceiveKeyboardWillShowNotification:(NSNotification*)notification
{
    ...
    //DEBUGLog(@"origin = %f %f",origin.x,origin.y);
    ...
    frame.size.height = popupViewHeight+30;
...
- (void)updateCount
{
    NSInteger textCount = [self.text length];
    _countLabel.text = [NSString stringWithFormat:@"%ld", (long)(_maxCount-textCount)];
